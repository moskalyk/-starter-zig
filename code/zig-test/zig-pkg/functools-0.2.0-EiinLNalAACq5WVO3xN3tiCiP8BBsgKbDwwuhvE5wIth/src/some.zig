...
const rangeArrayList = @import("range.zig").rangeArrayList;
...
test "test some array list" {
    ...
    var arr = try rangeArrayList(allocator, i32, 4);
...
test "test every on Point2D array list" {
    const allocator = testing.allocator;
    var arr = try ArrayList(Point2D).initCapacity(allocator, 3);
    defer arr.deinit(allocator);

    arr.appendAssumeCapacity(.{ .x = 5, .y = 2 });
    arr.appendAssumeCapacity(.{ .x = 1, .y = 3 });
    arr.appendAssumeCapacity(.{ .x = 0, .y = 4 });
